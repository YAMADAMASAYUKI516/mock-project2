...
public function detailByDate($user_id, $date)
{
    ...
    $requestData = AttendanceRequest::whereHas('attendance', function ($q) use ($user_id, $date) {
    ...
    public function staff($id, Request $request)
    {
        ...
        $requests = DB::table('requests')
            ...
            ->whereBetween(DB::raw('COALESCE(attendances.work_date, requests.target_date)'), [$startOfMonth, $endOfMonth])
            ->select(
                'requests.*',
        ...
        foreach ($requests as $dateKey => $requestRow) {
            if (!isset($attendances[$dateKey])) {
                ...
                if ($attendance->start_time && $attendance->end_time) {
                    ...
                    $attendance->total_time_formatted = sprintf('%d:%02d', intdiv($totalMinutes, 60), $totalMinutes % 60);
                } else {
    ...
    public function exportCsv($id, Request $request)
    {
        ...
        foreach ($datesInMonth as $date) {
            ...
            if ($startTime && $endTime) {
                ...
                $totalTime = sprintf('%d:%02d', intdiv($totalMinutes, 60), $totalMinutes % 60);
            }
